"""
训练脚本 — FusionModelOpt: SBP_opt(17-dim) + DBP_opt(12-dim) 最优子集融合

每个 BP 独立最优特征子集 + 独立 MLP 投影，concat 到各自 context。
对比全 169 维融合，验证特征精选是否优于满维冗余。

保存: cache/fusion_opt_fold_X.pkl
日志: log_train_fusion_opt

用法:
    conda run -n Pytorch python train_fusion_opt.py
"""
...
class FusionOptConfig(Config):
    def __init__(self):
        ...
        logger_init(log_file_name='log_train_fusion_opt', log_level=logging.INFO, log_dir=self.model_save_dir)
...
def train_fold(cfg, fold):
    ...
    model = FusionModelOpt(filters=cfg.filters, num_layers=cfg.num_layers, drop_prob=cfg.dropout)
    save_path = os.path.join(cfg.model_save_dir, f'fusion_opt_fold_{fold}.pkl')
    model = model.to(cfg.device)

    loss_fn = torch.nn.MSELoss()
    opt = torch.optim.Adam(model.parameters(), lr=cfg.lr, weight_decay=cfg.weight_decay,
                           betas=(cfg.beta1, cfg.beta2), eps=cfg.epsilon)
    sched = torch.optim.lr_scheduler.ReduceLROnPlateau(opt, mode='min', factor=0.5, patience=6, min_lr=1e-6, verbose=True)
    ...
    for ep in range(cfg.epochs):
        ...
        t0 = time.time()
        for (x_p, x_s, x_d), y in tqdm(train_iter, total=len(train_iter), desc=f"Epoch {ep}", colour="cyan"):
            x_p = x_p.to(cfg.device); x_s = x_s.to(cfg.device); x_d = x_d.to(cfg.device); y = y.to(cfg.device)
            opt.zero_grad()
            o = model(x_p, x_s, x_d)
...
def evaluate(dev, val_iter, loss_fn, model, s_std, d_std):
    ...
    with torch.no_grad():
        for (x_p, x_s, x_d), y in tqdm(val_iter, total=len(val_iter), desc="Eval", colour="cyan"):
            x_p=x_p.to(dev); x_s=x_s.to(dev); x_d=x_d.to(dev); y=y.to(dev)
            o = model(x_p, x_s, x_d)
...
if __name__ == '__main__':
    cfg = FusionOptConfig()
    metrics_acc = {k:[] for k in ['mae','sbp_mae','dbp_mae','sbp_mmhg','dbp_mmhg']}
    for fold in range(cfg.k):
        m = train_fold(cfg, fold)
        (_, _, _, _, _, vmae, vsmae, vdmae, vsmae_m, vdmae_m) = m
        metrics_acc['mae'].append(vmae); metrics_acc['sbp_mae'].append(vsmae)
        metrics_acc['dbp_mae'].append(vdmae); metrics_acc['sbp_mmhg'].append(vsmae_m)
        metrics_acc['dbp_mmhg'].append(vdmae_m)
        n = len(metrics_acc['mae'])
        logging.info(f"{n} fold SBP MAE = {np.mean(metrics_acc['sbp_mmhg']):.3f} ± {np.std(metrics_acc['sbp_mmhg']):.3f} mmHg")
        logging.info(f"{n} fold DBP MAE = {np.mean(metrics_acc['dbp_mmhg']):.3f} ± {np.std(metrics_acc['dbp_mmhg']):.3f} mmHg")

    logging.info("="*60)
    logging.info("FINAL 5-FOLD CV (FusionModelOpt)")
    logging.info(f"SBP MAE = {np.mean(metrics_acc['sbp_mmhg']):.2f} ± {np.std(metrics_acc['sbp_mmhg']):.2f} mmHg")
    logging.info(f"DBP MAE = {np.mean(metrics_acc['dbp_mmhg']):.2f} ± {np.std(metrics_acc['dbp_mmhg']):.2f} mmHg")
    print(f"\n===== FusionModelOpt 5-Fold CV =====")
    print(f"SBP MAE = {np.mean(metrics_acc['sbp_mmhg']):.2f} ± {np.std(metrics_acc['sbp_mmhg']):.2f} mmHg")
    print(f"DBP MAE = {np.mean(metrics_acc['dbp_mmhg']):.2f} ± {np.std(metrics_acc['dbp_mmhg']):.2f} mmHg")
