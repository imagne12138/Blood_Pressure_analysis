"""
训练脚本 — FusionModel169: PPG 时序路 + 169-dim Liu2023 特征融合

模型:
  - FusionModel169 (CNN-BiLSTM-Attention + 169-dim 特征 MLP 投影后 concat)
  - 投影维度: 169→64
  - 融合维度: 256 + 64 = 320
  - 分头注意力 + 独立 SBP/DBP 输出头

保存:
  - 模型: cache/fusion169_fold_X.pkl
  - 日志: log_train_fusion169

用法:
    conda run -n Pytorch python train_fusion_169.py
"""
import logging
import os
import time
...
class Fusion169Config(Config):
    """Config overrides for 169-dim fusion model training."""
    def __init__(self):
        super().__init__()
        # 清除父类创建的 log_train handler
        _old_paths = []
        ...
        for _p in _old_paths:
            if os.path.exists(_p):
                os.remove(_p)
        # 重新初始化
        logger_init(log_file_name='log_train_fusion169',
...
def train_fold(cfg, fold):
    ...
    logging.info("############ 初始化 FusionModel169 ############")
    model = FusionModel169(
        filters=cfg.filters,
    ...
    model_save_path = os.path.join(cfg.model_save_dir, f'fusion169_fold_{fold}.pkl')
...
if __name__ == '__main__':
    ...
    logging.info("=" * 60)
    logging.info("FINAL 5-FOLD CROSS-VALIDATION RESULTS (FusionModel169)")
    logging.info("=" * 60)
    logging.info(
    ...
    print(f"\n===== FusionModel169 5-Fold CV Results =====")
    ...
    print(f"DBP RMSE = {np.sqrt(np.mean(all_fold_DBP_loss_mmHg)):.2f} ± "
          f"{np.std(np.sqrt(np.array(all_fold_DBP_loss_mmHg))):.2f} mmHg")
    print(f"Models saved to: {cfg.model_save_dir}/fusion169_fold_*.pkl")
