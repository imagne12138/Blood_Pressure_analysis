"""
训练脚本 — FusionModel26: PPG 时序路 + 26-dim 统计特征融合

基于 train.py (Model_2_Head 训练流程) 改造:
  - 数据: 同时加载 segmented_records.h5 (PPG) 和 ppg_features.h5 (26-dim 特征)
  - 模型: FusionModel26 (CNN-BiLSTM-Attention + 特征 concat)
  - 保存: cache/fusion26_fold_X.pkl
  - 日志: log_train_fusion26

用法:
    conda run -n Pytorch python train_fusion_26.py
"""
import logging
import os
import time
...
class Fusion26Config(Config):
    """Config overrides for 26-dim fusion model training."""
    def __init__(self):
        # 先调用父类初始化（含 logger_init('log_train')，会创建 log_train 文件 + handlers）
        super().__init__()
        # 清除父类挂载的 root handler，否则第二次 logger_init 被 basicConfig 静默忽略
        import os as _os
        _old_paths = []
        ...
        # 删除父类创建的空 log_train 文件
        for _p in _old_paths:
            if _os.path.exists(_p):
                _os.remove(_p)
        # 用正确的日志名重新初始化
        logger_init(log_file_name='log_train_fusion26',
...
def train_fold(cfg, fold):
    ...
    logging.info("############ 初始化 FusionModel26 ############")
    model = FusionModel26(
        filters=cfg.filters,
    ...
    model_save_path = os.path.join(cfg.model_save_dir, f'fusion26_fold_{fold}.pkl')
...
if __name__ == '__main__':
    ...
    for fold in range(cfg.k):
        ...
        n = len(all_fold_mae)
        logging.info(
            f"{n} fold MAE: {np.mean(all_fold_mae):.3f} ± {np.std(all_fold_mae):.3f}"
        )
        logging.info(
            f"{n} fold SBP MAE: {np.mean(all_fold_SBP_mae):.3f} ± "
            f"{np.std(all_fold_SBP_mae):.3f}"
        )
        logging.info(
            f"{n} fold DBP MAE: {np.mean(all_fold_DBP_mae):.3f} ± "
            f"{np.std(all_fold_DBP_mae):.3f}"
        )
        logging.info(
            f"{n} fold SBP MAE in mmHg: {np.mean(all_fold_SBP_mae_mmHg):.3f} ± "
            f"{np.std(all_fold_SBP_mae_mmHg):.3f}"
        )
        logging.info(
            f"{n} fold DBP MAE in mmHg: {np.mean(all_fold_DBP_mae_mmHg):.3f} ± "
            f"{np.std(all_fold_DBP_mae_mmHg):.3f}"
        )

    logging.info("=" * 60)
    logging.info("FINAL 5-FOLD CROSS-VALIDATION RESULTS (FusionModel26)")
    logging.info("=" * 60)
    logging.info(
        f"SBP MAE = {np.mean(all_fold_SBP_mae_mmHg):.2f} ± "
        f"{np.std(all_fold_SBP_mae_mmHg):.2f} mmHg"
    )
    logging.info(
        f"DBP MAE = {np.mean(all_fold_DBP_mae_mmHg):.2f} ± "
        f"{np.std(all_fold_DBP_mae_mmHg):.2f} mmHg"
    )
    logging.info(
        f"SBP RMSE = {np.sqrt(np.mean(all_fold_SBP_loss_mmHg)):.2f} ± "
        f"{np.std(np.sqrt(np.array(all_fold_SBP_loss_mmHg))):.2f} mmHg"
    )
    logging.info(
        f"DBP RMSE = {np.sqrt(np.mean(all_fold_DBP_loss_mmHg)):.2f} ± "
        f"{np.std(np.sqrt(np.array(all_fold_DBP_loss_mmHg))):.2f} mmHg"
    )

    print(f"\n===== FusionModel26 5-Fold CV Results =====")
    print(f"SBP MAE = {np.mean(all_fold_SBP_mae_mmHg):.2f} ± "
          f"{np.std(all_fold_SBP_mae_mmHg):.2f} mmHg")
    print(f"DBP MAE = {np.mean(all_fold_DBP_mae_mmHg):.2f} ± "
          f"{np.std(all_fold_DBP_mae_mmHg):.2f} mmHg")
    print(f"SBP RMSE = {np.sqrt(np.mean(all_fold_SBP_loss_mmHg)):.2f} ± "
          f"{np.std(np.sqrt(np.array(all_fold_SBP_loss_mmHg))):.2f} mmHg")
    print(f"DBP RMSE = {np.sqrt(np.mean(all_fold_DBP_loss_mmHg)):.2f} ± "
          f"{np.std(np.sqrt(np.array(all_fold_DBP_loss_mmHg))):.2f} mmHg")
    print(f"Models saved to: {cfg.model_save_dir}/fusion26_fold_*.pkl")
