...
from model.MLP_for_feature import MLP
from utils.create_data import LoadPPGFeatureDataset
...
def train_model(cfg, fold):
    logging.info("############载入特征数据集############")
    data_loader = LoadPPGFeatureDataset(batch_size=cfg.batch_size)

    fold_dir = os.path.join(cfg.base_dir, f"cv_fold_{fold}.npz")
    train_iter, val_iter, fold_sbp_mean, fold_sbp_std, fold_dbp_mean, fold_dbp_std = data_loader.load_train_val_data(
        feature_path=str(cfg.h5_feature),
        label_path=str(cfg.h5_seg),
        indices_dir=fold_dir
    )

    logging.info("############初始化MLP模型############")
    model = MLP(in_feature_dim=cfg.feature_dim, hidden_dim=13, out_dim=2)
