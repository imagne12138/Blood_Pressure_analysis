"""
训练脚本 — FusionGated (26-dim): 门控融合 26 维特征

保存: cache/fusion_gated26_fold_X.pkl
日志: log_train_fusion_gated26

用法:
    conda run -n Pytorch python train_fusion_gated26.py
"""
...
class Gated26Config(Config):
    def __init__(self):
        ...
        logger_init(log_file_name='log_train_fusion_gated26', log_level=logging.INFO, log_dir=self.model_save_dir)
        self.feature_dim = 26
        self.proj_dim = 16
...
def train_fold(cfg, fold):
    ...
    dl = LoadFusion26Dataset(batch_size=cfg.batch_size)
    fold_dir = os.path.join(cfg.base_dir, f"cv_fold_{fold}.npz")
    train_iter, val_iter, sbp_m, sbp_s, dbp_m, dbp_s = dl.load_train_val_data(
        ppg_path=str(cfg.datadir), feat_path=str(cfg.h5_feature), indices_dir=fold_dir)
    ...
    save_path = os.path.join(cfg.model_save_dir, f'fusion_gated26_fold_{fold}.pkl')
...
def evaluate(dev, val_iter, loss_fn, model, s_std, d_std, fold=None):
    ...
    if fold is not None:
        ...
        gate_sbp = torch.cat(gate_sbp_list, dim=0) if len(gate_sbp_list) > 0 else torch.zeros(0, 16)
        gate_dbp = torch.cat(gate_dbp_list, dim=0) if len(gate_dbp_list) > 0 else torch.zeros(0, 16)
        # gate_sbp/dbp shape: [num_batches * proj_dim, proj_dim] from weight matrix
        # 实际上 weight 是 [proj_dim, 256]，我们看每行的均值（gate对256个context维的平均响应）
        w_sbp = model.sbp_gate.weight.data.cpu().numpy()  # [16, 256]
        w_dbp = model.dbp_gate.weight.data.cpu().numpy()  # [16, 256]
        fig, axes = plt.subplots(1, 2, figsize=(12, 5))
        axes[0].bar(range(16), w_sbp.mean(axis=1))
        ...
        axes[1].bar(range(16), w_dbp.mean(axis=1))
        ...
        plt.savefig(fr'E:\Kaggle_projects\Blood_Pressure_analysis\cache\figures\gate_pattern_fold{fold}.png', dpi=150)
        plt.close()
        # 计算 gate weight 的余弦相似度看两个 gate 是否一致
        w_sbp_flat = w_sbp.ravel()
        w_dbp_flat = w_dbp.ravel()
...
if __name__ == '__main__':
    ...
    logging.info("="*60)
    logging.info("FINAL 5-FOLD CV (FusionGated26)")
