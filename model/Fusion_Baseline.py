"""
BaselineModel + 169-dim Liu2023 feature fusion.
共享注意力 + 共享输出头，SBP/DBP 共用一套参数。
比 FusionModel169（分头方案）参数更少，理论上对跨折泛化更鲁棒。
"""
import torch
import torch.nn as nn


class FusionBaseline169(nn.Module):
    """
    时序路: Conv1D(1→32→64→128) → BiLSTM(hidden=128, bi) → 共享注意力 → 256-dim context
    特征路: 169-dim Liu2023 → MLP(169→64)
    融合:   concat(256, 64) → 共享输出头 Linear(320→128→ReLU→Dropout→2)

    forward inputs:
        x:        [batch_size, 1, 1024]  — PPG 原始信号
        features: [batch_size, 169]      — Liu2023 特征
    returns:
        [batch_size, 2]                  — [SBP_pred, DBP_pred]
    """

    def __init__(self, filters, num_layers, num_directions=2, hidden_dim=128,
                 drop_prob=0.2, attn_out_dim=1, proj_dim=64):
        super(FusionBaseline169, self).__init__()
        ...
        # ---- 共享输出头: SBP/DBP 一起输出 ----
        fusion_dim = self.hidden_dim * self.num_directions + self.proj_dim  # 256 + 64 = 320
    ...
    def forward(self, x, features):
        ...
        # ---- 特征投影后融合 ----
        proj_feature = self.feature_projection(features)  # [B, 64]
        c = torch.cat([c, proj_feature], dim=1)            # [B, 320]
